...
struct BINARYNINJAUIAPI DockProperties
{
	DockProperties() {};
	DockProperties(
	    QDockWidget* dw, bool vis, Qt::DockWidgetArea dar, Qt::Orientation dor, bool dvis, bool vs) :
	    dockWidget(dw),
	    visibleState(vis), defaultArea(dar), defaultOrientation(dor), defaultVisibility(dvis),
	...
	DockProperties(const DockProperties& dp) :
	    ...
	    viewSensitive(dp.viewSensitive), neverBeenVisible(dp.neverBeenVisible),
	    sizeStash(dp.sizeStash), actionOnShow(dp.actionOnShow)
	{}
...
class BINARYNINJAUIAPI DockHandler : public QObject
{
	...
	std::map<QString,
	    std::pair<QString, std::function<QWidget*(const QString&, ViewFrame*, BinaryViewRef)>>>
	    m_widgetFactories;
	DockContextHandler* getDockContextHandler(QWidget* widget);
	bool addDockWidget(const QString& name, QWidget* widget, Qt::DockWidgetArea area,
	    Qt::Orientation orientation, bool defaultVisibility);
 ...
 public:
	...
	bool addDockWidget(const QString& name,
	    ...
	    Qt::Orientation orientation = Qt::Horizontal, bool defaultVisibility = false);
	void createDynamicWidgets(ViewFrame* frame, BinaryViewRef data,
	    std::map<QString, QPointer<QWidget>>& store, std::vector<QString> filter,
